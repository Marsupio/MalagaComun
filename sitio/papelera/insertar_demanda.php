...
{
?>
<h2 align="left">DEMANDAS: Publica tu anuncio</h2>


<form  id="formulario"  action="insertar_demanda.php" method="POST" lang="es" enctype="multipart/form-data" >

<span id="label_formulario">Título</span><br/>
<input id="formulario"  name="titulo" type="text" required />

<span id="label_formulario">Descripción</span><br/>
<textarea id="formulario" name="cuerpo"  rows="7" required /></textarea>
...
<button class="button" id="boton"   type="submit">Publicar Demanda</button>	
...
/************ sube la imagen ********************************************************/
	...
	if (is_uploaded_file($tmpimagen)) 
	{
		...
		{			
			...
			$urlnueva = "imagenes/anuncios/demandas/".$nameimagen; 				// para la base de datos
			
			$original = "imagenes/anuncios/demandas/".$nameimagen; 				// ruta imagen original
			$copia = "imagenes/anuncios/demandas/miniatura_".$nameimagen;		// ruta miniatura
...
mysqli_query($con, "insert into demandas (TITULO,CUERPO,CATEGORIA,EMAIL,FECHA,LOCALIZACION,FOTO) values ('$titulo','$cuerpo','$categoria','$email','$fecha', '$localizacion', '$urlnueva')");
	
	mysqli_close($con);


	echo'<p>Se ha insertado su demanda correctamente en la categoría correspondiente.<br>Ahora puede insertar otra demanda o seguir navegando por el sitio.</p>';
              
	echo '<a href="mis_anuncios.php" class="button">Volver</a>';
	}
	else
	{
		echo '<p>Por favor, asegúrate de que has escrito correctamente tu email y vuelve a intentarlo. Gracias.</p>';
	}
