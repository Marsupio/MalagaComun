...
echo "<tr>  <th>Anuncio nº ".$id."<br/><h3 style='color:white;'>". $titulo ."</h3></th> </tr>";
...
<div align="center">
        <img  id="imagen_anuncio"  src="<?php echo $ruta_miniatura ?>" />
        <br/>
...
echo "<tr><td>Localidad: ".$localizacion."<br/>Publicado el ".$fecha."</td></tr>";	
				
echo "<tr> <td><a class='button'  href='responder_anuncio.php?para=$email&nombre_completo=$nombre_completo'>Enviar Correo</a>";
echo "&nbsp;&nbsp;&nbsp;";
echo "<a class='button'  href='ver_perfil_usuario.php?email=$email'>Ver perfil de este usuario</a></td></tr>";	
...
function muestra_anuncio($id,$titulo,$cuerpo,$email,$categoria,$fecha,$tipo_anuncio,$id_usuario,$nombre_completo,$localizacion,$telefono)
{
	...
	<button>Contactar</button></a></td></tr>";
...
function muestra_titulo_anuncios()
{
	...
	$rs_ofertas=mysql_query('SELECT * FROM ofertas',$conexion);
	$rs_demandas=mysql_query('SELECT * FROM demandas',$conexion);
	...
	while ($fila=mysql_fetch_array($rs_ofertas))
	{	
	...
	while ($fila=mysql_fetch_array($rs_demandas))
	{	
	...
	mysql_free_result($rs_ofertas);
	mysql_free_result($rs_demandas);
...
{	
    ...
    echo "<tr> <td>Comentario: <br/> " . $comentario . "</td></tr>";
...
echo '<tr style="background-color:black; color: white;"><td>Recibo nº '. $id .'<br/><a class="button"  href="muestra_recibo.php?id='.$id.'&pagador='.$pagador.'&email_pagador='.$email_pagador.'&titulo='.$titulo.'&receptor='.$receptor.'&email_receptor='.$email_receptor.'&cantidad='.$cantidad.'&comentario='.$comentario.'&fecha='.$fecha.' ">Detalles</a></td></tr>';

echo "<tr style='text-align:left'>";

echo '<tr><td>Emisor: '. $pagador .'<br/><a class="button"  href="consulta2.php?titulo_del_pago='.$titulo.'&pagador_del_pago='.$pagador.'">Historial</a></td></tr>';
echo '<tr><td>'.$cantidad .' comun(es) <br/>'. $fecha .'</td></tr>';
...
/*****************************************************************************************************/
function enviar_email_aviso_pago($email_receptor,$pagador,$cantidad,$titulo,$email_pagador)
{
...
$mensaje = "<h1>¡Enhorabuena! </h1> <br><h2> Ha recibido un pago de ".$cantidad." comune(s) de ".$pagador."</h2><br><p>Revise su pagos recibidos en la web para más detalles</p>";
/*
$mensaje = "Mensaje desde Malaga Comun. \r\n";
$mensaje .= "Este mensaje fue enviado por: " . $pagador . "\r\n";
$mensaje .= "En concepto de pago al anuncio: " . $titulo . " \r\n";
$mensaje .= "Por una cantidad de: " . $cantidad .' comunes.'. "\r\n";
$mensaje .= "Pago efectuado el " . date('d/m/Y', time());
*/

$asunto = 'Pago realizado a su favor desde Malaga Comun';
...
{
	$conexion=conectar_base_datos();
	$rs = mysql_query("SELECT * FROM usuarios WHERE EMAIL='$email_receptor'");
...
{
	$conexion=conectar_base_datos();
	$rs = mysql_query("SELECT * FROM usuarios WHERE EMAIL='$email'");
	$fila=mysql_fetch_array($rs);
...
{
	$conexion=conectar_base_datos();
	$rs = mysql_query("SELECT * FROM pagos WHERE EMAIL_RECEPTOR='$email'");

	$pagos_recibidos = mysql_num_rows ( $rs );
...
{
	$conexion=conectar_base_datos();
	mysql_query("insert into pagos (PAGADOR,EMAIL_PAGADOR,TITULO,RECEPTOR,EMAIL_RECEPTOR,CANTIDAD,COMENTARIO,FECHA,VALORACION) values ('$pagador','$email_pagador','$titulo','$receptor','$email_receptor','$cantidad','$comentario','$fecha','$valoracion')");
	
}
/*******************************************************************************************************************/
function muestra_usuario_por_id($id)
{	
$registro = mysql_query("SELECT * FROM  usuarios WHERE ID='$id' ");

$row78 = mysql_fetch_array($registro);
...
//		$mensaje .= "Su e-mail es: " . $email . " <br />";
		...
		$mensaje =  " <br />" . utf8_decode($cuerpo_mensaje) . " <br />" . " <br />";
		$mensaje .= "Enviado el " . date('d/m/Y', time());
		
		$asunto = utf8_decode('Boletín de Ofertas y Demandas');
...
function mostrar_resultados_busqueda_usuario($buscar)
{
	...
	{	
	  ...
	  echo "<tr>";
	  echo "<td>Número: " . $id . ' <br />Nombre: '.$nombre.'<br />Apellidos: '.$apellidos.'<br />Teléfono:  '.$telefono.'<br />Email:  '.$email.' <br />Localidad: '.$localizacion.' <br />'."<a class='button' href='mostrar_usuario.php?email=$email'>Examinar</a></td>"; 
	  echo "</tr>";
	}
  	echo '</table>';
	}
...
function mostrar_resultados_busqueda_pagos($buscar)
{
	...
	{	
	  ...
	  echo "<tr><td>Usuario nº " . $id . "<br/>Nombre: " . $nombre . " " . $apellidos . "<br/>Teléfono: " . $telefono . "<br/>Email: " . $email ."<br/>Localidad: " . $localizacion ."<br/><a class='boton' href='consulta3.php?email=$email' >RECIBOS</a></td></tr>";
...
function calcula_reputacion($email)
{
	...
	$result = mysql_query($sql, $con);
	$total_pagos = mysql_num_rows($result);
...
echo "<tr> <td>Intercambios realizados</td><td>".$intercambios."</td></tr>";
	...
	if ($comunes<0)
	{
	  echo '<td style="font-size:30px; font-weight:bold; color:#a00; text-shadow: 2px 2px #aaa;">'.$comunes.'</td>'; 
	}
	elseif ($comunes>0)
	{
...
<a class='button'  href='presentar_hoja_pago.php?id=$id&&nombre=$nombre&&apellidos=$apellidos&&telefono=$telefono&&email=$email&&localizacion=$localizacion'>Pagar</a>
...
{
	$conexion=conectar_base_datos();
	$result = mysql_query("SELECT CP FROM usuarios");	
...
function crea_tabla_desde_fichero()
{
			...
			foreach ($array_fichero as $valor)
			{
				...
				mysql_query("INSERT INTO coordenadas (CP, LATITUD, LONGITUD) VALUES ('$cp','$latitud','$longitud')");	
...
function crea_coordenadas()
{
	...
	$total=count($codigos);
			...
			{
					$cp=$codigos[$i];
					$rs=mysql_query("select * from coordenadas where cp='$cp' ");
